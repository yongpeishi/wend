...
Previous accents straw `#C98A1E` and indigo `#3B4E80` are retired; the aliases have been removed. All work uses `--wend-apricot` and `--wend-plum`. Bister #55402F is retired — at 9.3:1 it weighed the same as deep leaf; murasaki plum #754E75 sits at 6.6:1 on card, between rust (5.9) and body (7.6). All text tokens clear WCAG AA against paper; ratios are recorded on the colour cards.
...
- **The accent change is not yet applied to the Roadbook kit.** `ui_kits/roadbook/` inherits `--wend-apricot` / `--wend-plum` from the tokens automatically, but its README and comments still say straw and indigo. Say the word and I'll rewrite the kit's own text.
