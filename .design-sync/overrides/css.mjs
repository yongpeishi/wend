// forked from design-sync lib/css.mjs — Wend's tokens live in the app's src/design tree, not an npm package, so copyTokens must accept a package-relative tokensPkg that has no package.json of its own.
...
export function copyTokens({ tokensPkg, tokensGlob, nodeModules, out }) {
  ...
  let tjson = {}; try { tjson = JSON.parse(readFileSync(join(tdir, 'package.json'), 'utf8')); } catch { /* not an npm package — a plain token dir */ }
  if (tokensGlob) {
  ...
  console.error(`  tokens: ${tokenFiles.length} files from ${tokensPkg}@${tjson.version ?? 'src'}`);
  return tokenFiles;
}
