...
describe('sortOpenTodos', () => {
  ...
  it('breaks a due-date tie in favour of the todo whose entry is scheduled', () => {
    const todos = [
      makeTodo({ id: 1, due_on: '2026-11-05', entry: { id: 10, kind: 'idea', title: 'Onsen', category: 'activity' } }),
      makeTodo({ id: 2, due_on: '2026-11-05', entry: { id: 11, kind: 'idea', title: 'Ryokan', category: 'lodging' } }),
    ];
...
describe('isEntryScheduled', () => {
  ...
  it('checks membership in the scheduled id set', () => {
    expect(isEntryScheduled({ id: 1, kind: 'idea', title: 'x', category: null }, new Set([1]))).toBe(true);
    expect(isEntryScheduled({ id: 2, kind: 'idea', title: 'x', category: null }, new Set([1]))).toBe(false);
  });
});
